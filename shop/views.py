...
def index(request):
    ...
    if email:
        ...
        mobiles_category = Category.objects.filter(category_name='Mobiles').first()
        recently_added_mobile_products = Product.objects.filter(category_id=mobiles_category).order_by('-created')[:4]
        
        camera_category = Category.objects.filter(category_name='Camera').first()
        recently_added_camera_products = Product.objects.filter(category_id=camera_category).order_by('-created')[:4]
        ...
        return render(request, 'shop/index.html', {
            ...
            'recently_added_mobile_products' : recently_added_mobile_products,
            'recently_added_camera_products' : recently_added_camera_products
        })
...
""" def searchMatch(query, item):
    ...
    category_id = str(item.category_id).lower()

    if re.search(pattern, description) or re.search(pattern, product_name) or re.search(pattern, category_id):
        return True
    else:
        return False
...
#     category_name = str(item.category_id).lower()
#     description = str(item.description).lower()
#     product_name = str(item.product_name).lower()
#     category_id = str(item.category_id).lower()
#     if re.search(pattern, category_name) or re.search(pattern, description) or re.search(pattern, product_name) or re.search(pattern, category_id):
#         return True
#     else:
#         return False
...
#             if searchMatch(query, category):
#                 matched_category = category
#                 break
            

#         if matched_category:
#             items = Product.objects.filter(category=matched_category)
#         else:
#             items = []
...
def search(request):
    if 'email' in request.session:
        ...
        allCategories = Category.objects.all()
        for category in allCategories:
            if searchMatch(query, category.category_name):
                matched_category = category
                break

        if matched_category:
            # If a category match is found, return all products in that category
            items = Product.objects.filter(category_id=matched_category)
        else:
            # If no category match, search in product details
            allProducts = Product.objects.all()
            items = [item for item in allProducts if searchMatch(query, item.description) or searchMatch(query, item.product_name) or searchMatch(query, str(item.category_id))]
